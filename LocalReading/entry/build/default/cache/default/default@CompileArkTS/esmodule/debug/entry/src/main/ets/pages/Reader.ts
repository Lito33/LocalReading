...
class Reader extends ViewPU {
    ...
    //监听“文本缩放因子”Display.scaledDensity的变化。检测到变化时重新初始化阅读器界面，以确保文字排版和UI布局能正确适配新的屏幕密度设置。
    ...
    private async loadCurrentPageText(resourceIndex: number) {
        ...
        try {
            ...
            if (spineItem.href)
                possiblePaths.push(spineItem.href); // 内容节点资源标识，唯一标识一个正文内容文件
            if (spineItem.idRef)
                possiblePaths.push(spineItem.idRef); // 内容资源在书籍内的引用路径
            // 如果 href 包含路径，有时需要去掉目录部分只取文件名？
            // 尝试不同的参数组合
